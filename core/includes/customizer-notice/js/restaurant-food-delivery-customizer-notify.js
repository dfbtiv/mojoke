...
(function (api) {
	...
	api.sectionConstructor['restaurant-food-delivery-customizer-notify-section'] = api.Section.extend(
		{
				...
				return true;
					...
					jQuery( document ).ready(
						function () {
							...
							jQuery( '.restaurant-food-delivery-customizer-notify-dismiss-recommended-action' ).click(
								function () {
									...
									jQuery.ajax(
										{
											type: 'GET',
											data: {action: 'restaurant_food_delivery_customizer_notify_dismiss_action', id: id, todo: action},
											dataType: 'html',
											url: restaurantfooddeliveryCustomizercompanionObject.ajaxurl,
											beforeSend: function () {
												jQuery( '#' + id ).parent().append( '<div id="temp_load" style="text-align:center"><img src="' + restaurantfooddeliveryCustomizercompanionObject.base_path + '/images/spinner-2x.gif" /></div>' );
											},
											success: function (data) {
												...
												var actions_count      = recommended_sction.find( '.restaurant-food-delivery-customizer-plugin-notify-actions-count' );
												var section_title      = recommended_sction.find( '.section-title' );
												jQuery( '.restaurant-food-delivery-customizer-plugin-notify-actions-count .current-index' ).text( index );
												container.slideToggle().remove();
												if (jQuery( '.restaurant-food-delivery-theme-recomended-actions_container > .epsilon-recommended-actions' ).length === 0) {

													actions_count.remove();

													if (jQuery( '.restaurant-food-delivery-theme-recomended-actions_container > .epsilon-recommended-plugins' ).length === 0) {
										...
										jQuery( '.restaurant-food-delivery-customizer-notify-dismiss-button-recommended-plugin' ).click(
											function () {
												var id = jQuery( this ).attr( 'id' ),
												action = jQuery( this ).attr( 'data-action' );
												jQuery.ajax(
										...
										function activatePlugin(url) {
										    ...
										    jQuery.ajax({
										        beforeSend: function() {
										            jQuery('.activate-now').replaceWith('<a class="button updating-message">' + restaurantfooddeliveryCustomizercompanionObject.activating_string + '...</a>');
										        },
										        async: true,
										        type: 'GET',
										        url: url,
 ...
 *
 * @package restaurant_food_delivery
 */
...
jQuery( document ).ready(
	function ($) {
		...
		$( 'body' ).on(
			'click', ' .restaurant-food-delivery-install-plugin ', function () {
